...
def initialize_db(mp4_filename):
    ...
    # .mp4 확장자를 제거하고 _lecture.pdf 확장자로 변경하여 output/pdf 경로에 설정
    pdf_filename = os.path.join('output', 'pdf', mp4_filename.replace('.mp4', '_lecture.pdf'))
    
    print(f"Checking if file exists: {pdf_filename}")  # 추가된 디버그 출력
